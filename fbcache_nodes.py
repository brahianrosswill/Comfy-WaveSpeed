...
class ApplyFBCacheOnModelStochastic:
    """
    Applies the First Block Cache mechanism to a model, optimized for use with
    stochastic samplers. This version ensures more rigorous cache validation
    by always comparing tensor values even for sequential evaluations at the
    same timestep. This may result in fewer cache hits compared to the standard
    version with stochastic samplers but provides more accurate caching by
    avoiding cache usage when first-block residuals differ due to sampler noise.
    """
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "residual_diff_threshold": (
                    "FLOAT",
                    {
                        ...
                        0.001,
                        "tooltip":
                        "Controls the tolerance for caching. Lower values are stricter. With stochastic samplers, the inherent variance might lead to fewer cache hits unless this threshold is adjusted. Setting to 0 disables FBCache.",
                    },
                ),
                "start": (
                    "FLOAT",
    ...
    CATEGORY = "wavespeed/stochastic" # Changed Category
    ...
    def patch(
        ...
        def reset_cache_state():
            ...
            # --- New lines to be added for Stochastic node ---
            current_ctx = first_block_cache.get_current_cache_context()
            if current_ctx is not None:
                current_ctx.force_value_comparison_for_sequence = True
            # --- End of new lines ---
...
NODE_CLASS_MAPPINGS = {
    ...
    "ApplyFBCacheOnModelStochastic": ApplyFBCacheOnModelStochastic,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ApplyFBCacheOnModel": "Apply First Block Cache",
    "ApplyFBCacheOnModelStochastic": "Apply First Block Cache (Stochastic)"
}
